...
-- 필요한 추가 데이터 X
-- 추가 조치 : 아이디로만 조회를 하고 암호화된 비밀번호 체크를 서버측에서 실행야함
...
-- 고객 상세보기
-- 최종 사용자 (고객 정보) => (고객번호, 고객 사진, 고객 이름, 생년 월일, 담당자 이름, 주소)
-- 고객 테이블에서 특정 고객을 조회하여 반환, 담당자 이름을 반환하기위해 영양사 테이블을 조인
-- 필요 추가 데이터 : 고객 번호 (화면에서)
SELECT C.customer_number, C.profile_image, C.name, C.birth, N.id, N.name, C.address
FROM customer C INNER JOIN nurse N
ON C.charger = N.id
